...
export function CreateContactForm({ onSubmit, isSubmitting }: CreateContactFormProps) {
  const [formValues, setFormValues] = useState<CreateContactInput>(initialValues);
  ...
  return (
    <form className="create-contact-form" onSubmit={handleSubmit}>
      <label>
        ...
        {errors.fullName?.[0] && <p role="alert">{errors.fullName[0]}</p>}
      </label>

      <label>
        <span>Email</span>
        <input name="email" value={formValues.email} onChange={handleChange} />
        {errors.email?.[0] && <p role="alert">{errors.email[0]}</p>}
      </label>

      <label>
        <span>Relationship</span>
        <select name="stage" value={formValues.stage} onChange={handleChange}>
          <option value="lead">Lead</option>
          <option value="qualified">Qualified</option>
          <option value="customer">Customer</option>
          <option value="inactive">Inactive</option>
        </select>
        {errors.stage?.[0] && <p role="alert">{errors.stage[0]}</p>}
      </label>

      <label>
        <span>Company</span>
        <input name="company" value={formValues.company} onChange={handleChange} />
        {errors.company?.[0] && <p role="alert">{errors.company[0]}</p>}
      </label>
